...
$(document).ready(function() {
  ...
  $('#searchText').keypress(function(e) { //keypress (or keydown) is better than keyup since we can suppress form submit on 'enter' key being pressed   
    ...
    var wikiLink = 'https://en.wikipedia.org/w/api.php?action=query&prop=extracts|info&exintro&exlimit=max&inprop=url&generator=search&gsroffset=&format=json&formatversion=2&callback=?&gsrsearch=' + searchItem + '&continue=';
    pageNum = 1;
 ...
 /************************************************************/
  $('.fa-search').on('click', function() {
    var wikiLink = 'https://en.wikipedia.org/w/api.php?action=query&prop=extracts|info&exintro&exlimit=max&inprop=url&generator=search&gsroffset=&format=json&formatversion=2&callback=?&gsrsearch=' + $('#searchText').val().replace(/\s+/g, '%20') + '&continue=';
    pageNum = 1;
  ...
  $('.displayResults').on('click', '.next', function() { //.displayResults is the stationary object into which the dynamically created .next is created
    ...
    var wikiLink = 'https://en.wikipedia.org/w/api.php?action=query&prop=extracts|info&exintro&exlimit=max&inprop=url&generator=search&format=json&formatversion=2&callback=?&gsrsearch=' + $('#searchText').val() + '&gsroffset=' + sroffset;
    wikiCall(wikiLink);
  });
  /************************************************************/
  $('.displayResults').on('click', '.back', function() { //.displayResults is the stationary object into which the dynamically created .back is created
    ...
    if (pageNum > 2) {
      pageNum -= 1;
      prev_sroffset -= diff;
      var wikiLink = 'https://en.wikipedia.org/w/api.php?action=query&prop=extracts|info&exintro&exlimit=max&inprop=url&generator=search&format=json&formatversion=2&callback=?&gsrsearch=' + $('#searchText').val() + '&gsroffset=' + prev_sroffset;
      wikiCall(wikiLink);
    } else if (pageNum === 2) {
      pageNum -= 1;
...
function wikiCall(link) {
  ...
  $.getJSON(link, function(searchResults) {
    if (typeof searchResults.continue === 'undefined') { //if there are no further search results to load
      ...
      if (pageNum === 1) {
        $(".displayResults").append("<span class='btn btn-success next'>Load More</span>");
      } else {
        $(".displayResults").append("<span class='btn btn-info back'><i class='fa fa-reply'></i> Back</span><span class='btn btn-success next'>Load More</span>");
      }
    }
